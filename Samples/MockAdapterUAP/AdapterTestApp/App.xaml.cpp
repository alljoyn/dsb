...
using namespace MockAdapterTestUAP;
...
App::App()
{
    ...
    {
        this->adapterTest = ref new MockAdapterTest(ref new MockAdapter());

        uint32 status = this->adapterTest->Initialize();
        if (status != ERROR_SUCCESS)
        {
            throw ref new Exception(status, "Mock Adapter Test initialization failed!");
        }
    }));
}
...
void App::OnLaunched(LaunchActivatedEventArgs^ e)
{
    ...
    {
        throw ref new Exception(E_INVALIDARG, "Mock Adapter Test not initialized!");
    }

    HRESULT hr = this->adapterTest->Start();
    if (FAILED(hr))
    {
        throw ref new Exception(hr, "Mock Adapter failed to start!");
    }
